...
- [`docs/reassessment/COVERAGE-GAP-GATHERING.md`](docs/reassessment/COVERAGE-GAP-GATHERING.md) — mechanical-grep re-scan of the raw transcripts for the 8 thinnest PROMOTE-CAUTIOUSLY verdicts above; 3 changed (one got worse, one strengthened, one fragmented further), 5 unchanged.
- [`docs/reassessment/COVERAGE-GAP-CLOSEOUT.md`](docs/reassessment/COVERAGE-GAP-CLOSEOUT.md) — **closed out 2026-08-25 at 25% (from 43%).** Five real entries landed in `soic-ladder-rules-v1.yaml` (`roe_lender-001`, `market_cap_floor-001`, `price_to_book_deep_value-001`, `ev_to_ebitda_context-001`, `price_to_cash_flow_context-001`), verified against a live 500-company judge run (13 verdicts changed, 2 real downgrades caught). The remaining 15 metrics need either a deliberate G6-precedent reversal or a search past this reassessment's 58-lecture corpus — both named, neither started.
- [`docs/reassessment/LOST-CONDITIONS-GATE-CLOSEOUT.md`](docs/reassessment/LOST-CONDITIONS-GATE-CLOSEOUT.md) — **closed out 2026-08-25.** Adjudicated the 29 Stage-3 findings that sit on actual GATES (not the 71 that sit on observations, which can never exclude a company). Result: 0 rulebook edits — but not a null finding. 12 of 21 distinct citations are real SOIC-stated conditions (including the original turnaround-waiver finding that started this reassessment) this rulebook's `{company, sector, is_lender}` attribute system genuinely cannot express yet; 5 are false positives from a detector defect (matches purely on shared metric name, blind to entry-vs-exit context); 4 confirm the current design is already correct. Names the real next step: extend `RESOLVABLE_ATTRIBUTE_KEYS` with a turnaround signal.
- [`docs/reassessment/LOST-CONDITIONS-OBSERVATION-CLOSEOUT.md`](docs/reassessment/LOST-CONDITIONS-OBSERVATION-CLOSEOUT.md) — **closed out 2026-08-25.** Adjudicated the other 71 findings, on observations (informational only). 3 `display_text` fields expanded after independent quote verification — most notably `pe_context-001`, the rulebook's single most-cited observation (30 citations), whose text was one bare sentence with zero mention of the bank/NBFC/insurer/real-estate/cement exclusions that show up almost verbatim across 4+ separate lectures.
- [`docs/reassessment/FABLE-REVIEW-CLOSEOUT.md`](docs/reassessment/FABLE-REVIEW-CLOSEOUT.md) — **closed out 2026-08-25.** An independent Fable-tier adversarial re-review of the two adjudications above, applying a bar they hadn't: every real condition must end as a gate fix or at least an observation. The "0 fixes" conclusion did not survive — found 7 real additions (headlined by `turnaround_context-001`, finally giving the original seed finding of this whole reassessment a home) plus 1 correction (a self-contradictory sentence Fable caught in a live file) plus 1 genuine new-gate candidate (`entry_vstop-001`, price above the weekly V-Stop as a required entry leg) that the owner explicitly chose to ship as an observation rather than a new exclusion rule. Also caught two factual errors in the prior pass's own reasoning. Zero fabricated citations found across ~50 independently re-verified quotes. **Every finding from Stage 3's original 100 has now been read by two independent model-tier passes before anything landed.**
- [`docs/reassessment/LEVEL4-KNOWLEDGE-GRAPH.md`](docs/reassessment/LEVEL4-KNOWLEDGE-GRAPH.md) — 2026-08-25. A `/graphify` knowledge graph (488 nodes, 1,686 edges, 15 communities; 96% EXTRACTED confidence) built over the "Stock Framework" Obsidian vault (145 files — lectures, rules, companies, findings). Confirmed against the actual Nate Herk transcript that this matches his "Level 4" (entity-relationship graph, distinct from Level 2's mere backlinks). Documents how it was generated (7 parallel subagents, one real ID-dedup gap left as a known imperfection), how to trace it (interactively via `graph.html`, or programmatically over `graph.json`), and — the load-bearing section — four concrete, grounded (not oversold) ways it could underpin an autonomous stock-picking agent as a citation/context retrieval layer, explicitly not as a verdict engine.
- [`docs/reassessment/LEVEL5-AGENT-MEMORY-DESIGN.md`](docs/reassessment/LEVEL5-AGENT-MEMORY-DESIGN.md) — 2026-08-26. **Design proposal, nothing built.** Drafted by Fable per the owner's request. Proposes Level 5 (Nate Herk's "always-on Brain OS," including his own warning that he doesn't run it himself) as agent memory for stock-picking, structured via an episodic/semantic/procedural framing: **Phase 1** (buildable now) is an append-only prediction ledger citing Level-4 graph nodes + a monthly outcome-scoring cron + a quarterly human-reviewed rulebook-diff proposal (reusing `evolve-frameworks`'s propose-only pattern) — grounded in Jack Roberts' traps (survivorship bias, <100-outcome overfitting, correlation-vs-index-baseline, hindsight contamination). **Phase 1.5** (a fresh proposal, not previously agreed) adds procedural "playbooks." **Phase 2** (forward-looking growth-stock scoring) is explicitly gated on ~100+ closed outcomes existing — stated plainly as not yet real. Also records a dependency check: promoter-holding is a real, cited FESTF gap (01:28:37-01:29:56) worth adding as an observation, with a reusable `parse_statement_section` fetch path flagged **unverified against live screener.in HTML**; commodity/gold/silver, broader shareholding-pattern, and bulk/block-deal signals are real but confirmed to live almost entirely outside the current 58-lecture corpus (in a ~1,115-file library) — owner decision: deferred, not re-litigated here.
- [`docs/reassessment/ms/SYNTHESIS.md`](docs/reassessment/ms/SYNTHESIS.md) — **2026-09-15. Market Signals series read end to end — the deferred corpus from the Level-5 dependency check, now done.** Pulled 12 new portal lessons first (9 with transcripts; 4 have none on the portal yet), then 39 Sonnet readers → `scripts/verify_briefs.py` (39/39 pass, 471/488 quotes verbatim; four transcripts have no timestamps and cite `(REF NO-TS)`, which the gate now maps to a presence-only check) → Fable synthesis. Headlines: (1) the **30-week-EMA + relative-strength entry legs are now stated in 13 distinct recordings across all three corpora** — the single most-corroborated finding of the whole reassessment, and `judge.py` already computes both metrics on the exit side, so an entry-side observation is a config change; (2) **promoter/shareholding behaviour (25 briefs) and sector rotation (23) have zero coverage** — holding %, pledge %, FII/DII % are `[HARD]` (on screener's shareholding page, unfetched), warrants/insider trades `[SOFT]`; (3) **G2 has one gate-shaped bright line**, negative CFO AND rising debt (MS0706B, corroborated by 3 more), blocked only on fetching CFO *sign* (only ratios exist today); (4) five growth-quality distortions with no observation home (inventory/commodity gains in 6 briefs, pass-through sales, tariff/restocking bounce, recurring one-offs, order-book degrowth); (5) **delivery % appears in none of the 39** — that signal is not in this corpus at all; (6) against the 48: doubts on NEULANDLAB (957% PAT is a base-effect artifact by SOIC's own reading), NAVINFLUOR (refrigerant-price leg), DIXON, cables trio, GLAND (promoter exit), MCX (prop-trading regulation); strongest supports ACUTAAS (7 briefs), SYRMA, USHAMART, APARINDS, COFORGE, HONASA. New false-positive: MS1207A's "KSH" is KSH International, not KEI. Two probable duplicate-provenance matches for the rulebook owner (MS1503A 01:10:45 ↔ `capital_efficiency-001`'s SOICA6; MS2806A 00:14:01 ↔ `cfo_to_pat-001`'s SOICC). Briefs in `ms/`, vault rebuilt (`scripts/build_framework_vault.py` now has an `ms` course: 97 lectures / 28 rules), Level-4 graph rebuilt over it (196 files → **557 nodes / 1,943 edges / 28 communities**, 97% EXTRACTED; 24 isolated ghost nodes from the older cache's different ID scheme were dropped — the same ID-dedup gap the Level-4 doc already names).

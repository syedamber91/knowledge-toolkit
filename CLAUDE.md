...
- [`docs/reassessment/COVERAGE-GAP-CLOSEOUT.md`](docs/reassessment/COVERAGE-GAP-CLOSEOUT.md) — **closed out 2026-08-25 at 25% (from 43%).** Five real entries landed in `soic-ladder-rules-v1.yaml` (`roe_lender-001`, `market_cap_floor-001`, `price_to_book_deep_value-001`, `ev_to_ebitda_context-001`, `price_to_cash_flow_context-001`), verified against a live 500-company judge run (13 verdicts changed, 2 real downgrades caught). Read this first for the current state; it links back to the three docs above for how it got there. The remaining 15 metrics need either a deliberate G6-precedent reversal or a search past this reassessment's 58-lecture corpus — both named, neither started. **Stage 3's 96 lost-condition findings remain unadjudicated — the largest unfinished piece of this whole reassessment.**

...
`output/`). A sibling Spark-internals pack and a Google Drive uploader live on
the `claude/eager-gates-UqFZF` branch (noted below).

| Script | Purpose | Output |
|--------|---------|--------|
| `scripts/generate_learning_pack.py` | Ben Dicken database-internals pack | `output/ben_dicken_phase1.pdf` |
| `scripts/generate_vutr_spark.py` *(eager-gates branch)* | Vu Trinh Spark-internals pack (5 chapters) | `output/vutr_spark.pdf` |
...
- **`vutr`, `lucsystemdesign`, `sdcourse`** *(eager-gates branch)* — additional
  examiners for Spark/Kafka/OLAP, system-design decisions, and distributed log
  processing.
...
(verification personas). The `claude/eager-gates-UqFZF` branch adds `vutr`,
`lucsystemdesign`, and `sdcourse` examiner agents. Note: agent files reference an
absolute project root from the author's machine — paths there are illustrative,
not this repo's path.

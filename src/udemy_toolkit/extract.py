...
# "00:00:04.000 --> 00:00:08.000 align:start position:10%" (full HH:MM:SS.mmm)
# or the equally-valid short form under an hour, "00:04.000 --> 00:08.000"
# (WebVTT allows omitting the hours group; Udemy's own autogenerated captions
# use this short form for any cue before the one-hour mark).
_TIMING_RE = re.compile(
    r"^(?:(?P<h>\d{2}):)?(?P<m>\d{2}):(?P<s>\d{2})[.,]\d{1,3}\s*-->\s*"
    r"(?:\d{2}:)?\d{2}:\d{2}"
)
...
def parse_cues(raw: str) -> List[Tuple[int, str]]:
    ...
    for line in (raw or "").splitlines():
        ...
        if match:
            ...
            start = hours * 3600 + int(match.group("m")) * 60 + int(match.group("s"))
            have_timing = True
            continue
        if not have_timing:

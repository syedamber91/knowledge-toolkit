...
- **Video mode** — a narrated MP4 (Remotion or Manim) where the screen shows each concept happening live while it is explained. Covered in `references/video-mode.md` — **read that file in full before starting a video**; the rest of this SKILL.md is written for page mode and its density/figure guidance does not apply to video the same way.
...
- **video** — a narrated MP4, as long as full coverage needs (a dense topic ran ~34 minutes, split into chapter-aligned episodes), built entirely on the pattern in `references/video-mode.md`: every source read first; each concept taught kid-version → live mechanism with the source's real numbers → real computer use case → weakness shown live → one-line takeaway; no bullet slides (pros, cons, best practices and trade-offs are animated vignettes); alternatives raced on the same input, then a scorecard and a decision tree; cue-level narration with visuals fired on the spoken word; a recall quiz and a one-screen memory map. This is a different deliverable shape from a page — do not treat it as "page content read aloud."
...
- `references/video-mode.md` — the full video pipeline: the per-concept teaching loop, show-don't-list rules, the race/scorecard/decision-tree comparison, the cue-level TTS + measured-segment timing contract, verified simulations, parallel scene building, still-based QA, episode cuts and delivery. Read it in full before starting any video — it is the whole skill for that mode, not a supplement to the sections above.

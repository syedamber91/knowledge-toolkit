...
> **Provenance (vendored 2026-09-21).** Copied verbatim from
> `github.com/tt-a1i/archify` @ `5289f68` (the `archify/` subdirectory —
> the repo's own packaged skill payload, `archify.zip`, minus the `test/`
> suite and the two dev-only generator scripts
> `scripts/generate-brand-marks.mjs` / `scripts/generate-validators.mjs`),
> skill version 2.17.0-dev.1, MIT, author tt-a1i (based on
> Cocoon-AI/architecture-diagram-generator, MIT v1.0). Vendored rather than
> installed via `npx skills add tt-a1i/archify -g` because that installer
> writes to the user's global skill directory, not into this repo, and
> would leave every fresh clone or cloud session without the skill — the
> same reason `ponytail` and `grill-me` are pinned here instead of relying
> on a live installer. `bin/archify.mjs` needs Node.js >= 18 on PATH to
> run diagram validation/rendering; `scripts/check-update.mjs`'s "Update
> awareness" step makes one network call to `tt-a1i.github.io` and
> degrades silently when offline (see the SKILL.md text below — "If the
> command cannot run, continue without mentioning the check"). To update,
> re-fetch a newer `archify.zip` from upstream and diff against this copy
> deliberately rather than re-running an installer against this pinned
> tree.
